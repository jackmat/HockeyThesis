...
GeneralTable<-GeneralTable%>%left_join(CompareGP[[i]][c('PlayerId','PlayerName', 'GP','GPsec')], 
  ...
  select(PlayerName,P, Age, Salary, GPsec, G,GA, PlusMin,NHL, Direct,Directh, Collective,Collectiveh)%>%
  filter(GPsec != 0)%>%
...
tableresData <- melt(TotalDataFrame, id.vars = colnames(TotalDataFrame)[c(1:7, length(colnames(TotalDataFrame)))])
