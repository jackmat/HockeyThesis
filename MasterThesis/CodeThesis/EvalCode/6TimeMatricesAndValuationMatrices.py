# -*- coding: utf-8 -*-
"""
Created on Tue Oct 23 12:41:52 2018
...
def main():
    ...
    for Season in range(2008,2015):
        print(Season)
#    Season = 2007
        PlayerValueMat = IndexByTeamSeason(Season,d,c)
        print("Value Matrix created")
#1        PlayerTimePlayer = IndexByTeamSeason(Season,d,c)
...
#1            TimeMat = PlayerTimePlayer[team] #change 0 to team
            ValueMat = PlayerValueMat[team]
            PlusMinMat = PlusMinus[team] 
            PlayerValueMat[team], PlusMinus[team]= ValuePlusMinusFunc(Team = Team , 
                          pandasframe = ValueMat,
                          pandasframe2 = PlusMinMat,
                          c = c ,
                          d = d)
#1            PlayerTimePlayer[team] = TimeCalculation(Team = Team, 
#1                                                        pandasframe3= TimeMat,
#1                                                       c = c,
#1                                                        d = d )
...
#1       PlayedTimePerMatch = JoinListpdMatrices(PlayerTimePlayer)        
        
        #(4) Store it to pass it to R 
        ValMat.to_csv(path + str(Season)+'PlayerValMatrix.csv')
#1        PlayedTimePerMatch.to_csv(path +str(Season)+ 'PlayerbyTimeValMatrix.csv') 
...
def TimeCalculation(Team, pandasframe3, c,d):
    ...
    #                    
    #    ###Return:
    #     #     It returns the same 1 pandasframe matrix with the time of each player played by match
    #                     
    # =============================================================================

    Matches = pandasframe3.GameId.values   
    Mat3 = pandasframe3
...
def ValuePlusMinusFunc(Team, pandasframe, pandasframe2, c,d):
    ### 
    # =============================================================================
    #     ### Description: For each match in the pd.Dataframe, select players and GameId, eventnum, typeEvent, EventTime
...
#match = 0        
        MatchId = int(Matches[match]) # change for match
        print(str(match)+" out of " + str(len(Matches)))
        Index = match
# =============================================================================
#         if Index >0:
#             break;
# =============================================================================
        ...
        try:
            ...
            print(MatchId, result, query, Team)
            break
        query = """SELECT EventNumber, EventType, PlayerId
                    FROM q_fulltable
                    WHERE GameId  = """ + str(MatchId) + \
                    " AND Event in " + ACTION_EVENTS + \
                    " AND Team = '""" + Venue +"' ORDER BY EventNumber"
        c.execute(query)
        EventIndex = c.fetchall()
        for i in range(len(EventIndex)):
#i = 2            
            ...
            PlayerId = EventIndex [i][2]
            # Selecting all Home-Away Players id
            PlayersPlayed = UniqueplayersperEvent(MatchId, Venue, d, EventNumber).astype(str).tolist()
            if PlayersPlayed != []:
                PlayersPlayed2 = [item for sublist in PlayersPlayed for item in sublist]
...
################################################## Need to check in this part the number
                    ...
                    try:
                        ...
                        print(Event, MatchId, EventNumber ,PlayerId, action_values, PlayerId, PlayersPlayed2, "problem!")
                        break
    return Mat, Mat2  
